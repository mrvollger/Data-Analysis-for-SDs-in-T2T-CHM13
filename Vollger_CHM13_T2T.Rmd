...
small = as.ggplot(expression(
  ...
  kpPlotRegions(kp,data=toGRanges(zoom_rgns), col=transparent("lightgreen",0.1), clipping = T, data.panel = 1, border = darker("lightgreen"))
  )
); small
#
...
flat_duplicon
probe_plot = cowplot::plot_grid(flat_duplicon, probe_l, nrow=2, rel_heights = c(8,1)) #labs(title = "Location of FISH probes on novel acrocentric duplications")
...
#fixed_dups_p
ggsave("supp/dup_fixed_dups.pdf", height = 10, width = 16, plot = probe_tbl)
...
#x=ggdraw()+draw_image("FISH/chm13_finaltable_14012021_copy_withlayers copy 2.png")#, x=0.1, y=0.1, height = .1, width = 1)
x=ggdraw()+draw_image("FISH/forMitchell_new_LM_19012021.tif")
flat_duplicon=cowplot::plot_grid(small + theme(plot.margin = unit(c(0, 0, 0, 0), "cm")),
                        flat_duplicon_1,
                        rel_heights = c(1,6), nrow=2)
flat_duplicon_lab = ggdraw() + draw_plot(flat_duplicon) + draw_plot(flat_dup_leg, x = .65, y=.3, height = .2,width = .3)

chr1_qcen=ggdraw()+draw_image("figures/chr1_qcen.png")#, x=0.1, y=0.1, height = .1, width = 1)


figure = cowplot::plot_grid(flat_duplicon_lab,
                   x,
                   cowplot::plot_grid(chr1_qcen,
                            NULL,
                            labels = c("c","d"),
                            rel_widths = c(3,1)),
                  labels = c("a","b",NA), nrow=3, rel_heights = c(3,1,1))
           
ggsave("supp/probe_ideogram.pdf", height=12, width=12, plot=flat_duplicon_lab)


scale = 1.5
ggsave("supp/Duplicons.pdf", plot=figure , width=12, height=12*scale) 
ggsave("supp/Duplicons.png", plot=figure , width=12*scale, height=6*scale, dpi = 300) 
...
s=1.2
ggsave("supp/intra_sd_ideo.pdf", width = 16*3/5*s, height = 12*s, plot=Intra)
ggsave("supp/intra_sd_ideo.png", width = 16*3/5*s, height = 12*s, plot=Intra, dpi = 600)
g_intra = ggdraw() + draw_image("supp/intra_sd_ideo.png")
...
f1 = ideo #plot_grid(ggdraw() + draw_plot(ideo) + draw_plot(f1_legend, .2, .2, .6, .3,  scale=2), 
      #         bot,
       #        nrow=2, 
        #       rel_heights = c(2,1))
...
ggsave("supp/intra_sd_ideo_legend.pdf", width = 3, height = 1, plot=f1_legend)
ggsave("figures/simple.f1.pdf", plot=ftall, height = 12*s, width = 16*s)

ggsave("supp/right_figure_1.pdf", width = 16*2.5/5*s, height = 12*s*2/5, plot=plot_grid(
                  sheall$identity,
                  p_duplicons,
                  rel_heights = c(1,1),
                  #labels = c("","c","d"),
                  ncol=1) )
ggsave("supp/right_figure_1_legend.pdf", plot=get_legend(by_all), width = 3, height = 1)
...
ggsave("supp/intercen.pdf", height = 12, width = 24, plot = fig)
fig
...
```{r fig.show='hide', message=TRUE, warning=TRUE, paged.print=FALSE}
...
# make seperate files for new SD and not
```{r}
ov1 = overlaps(SEDEF, NEW)
ov2 = overlaps(SEDEF[,c("chr2","start2","end2")], NEW)
newsds = ov1 | ov2


sedef = SEDEF
colnames(sedef)[1]="#chr"
write.table(sedef[newsds], file="../paired_sd_merge_test/New.sds.bed",
            row.names = F, col.names = T, sep = "\t", quote = F)
write.table(sedef[!newsds], file="../paired_sd_merge_test/Shared.sds.bed",
            row.names = F, col.names = T, sep = "\t", quote = F)
...
acrointer <- function(){
  # data
    sedef.inter <- SEDEF[chr != chr2 & original & acro & acro2 ]#[1:1000]
  ...
  circos.genomicLink(sedef.inter[,c("chr","start", "end")],
                     sedef.inter[,c("chr2","start2", "end2")],
                     border=NA, col=sedef.inter$color.y)
}; #Inter()
png("supp/inter_acro_circo.png", height = 6, width = 6, units="in", res=900, bg = "transparent")
acrointer()
dev.off()
pdf("supp/inter_acro_circo.pdf", height = 6, width = 6)
acrointer()
dev.off()
...
ggsave("supp/inter_acro_hist.pdf", height = 8, width = 8, plot=acro_hist)
acro_hist
```
