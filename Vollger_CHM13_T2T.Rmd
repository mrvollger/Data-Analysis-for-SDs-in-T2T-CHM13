...
small = as.ggplot(expression(
  ...
  kp<- plotKaryotype(genome=GENOME, plot.params = pp,
                     ...
                     plot.type = pt, cytobands = CYTOFULL, cex=1), #color.schema="only.centromeres"),
...
flat_dup_leg = as.ggplot(expression(
  ...
  kpText(kp,chr="chr14:0-14", x = c(min,min,min,min+1e5, min+3e5)-1.5e5, 
...
flat_duplicon = plot_grid(small, flat_duplicon_1, rel_heights = c(1,4), nrow=2)
flat_duplicon
...
flat_duplicon=plot_grid(small + theme(plot.margin = unit(c(0, 0, 0, 0), "cm")),
                        flat_duplicon_1,
...
figure = plot_grid(flat_duplicon_lab,
                   ...
                   plot_grid(chr1_qcen,
                            ...
                            labels = c("c","d"),
                            rel_widths = c(4,1)),
                  labels = c("a","b",NA), nrow=3, rel_heights = c(2,1,1.5))
           
scale = 1.5
ggsave("supp/Duplicons.pdf", plot=figure , width=12*scale, height=12*scale) 
ggsave("figures/Duplicons.png", plot=figure , width=12*scale, height=6*scale, dpi = 300) 
...
Intra=as.ggplot(expression(
  ...
  kpPlotRegions(kp, data=GenomicRanges::reduce(gi), col=transparent("orange", 0.25), r0=-.05, r1=0.05, border = darker("orange")),
  kpPlotLinks(kp, 
...
# make the legend
#
tmp=c("New or structurally variable SD in T2T CHM13",
      "SD shared with GRCh38",
      "Predicted genomic instability")#,
...
f1 = plot_grid(ggdraw() + draw_plot(ideo) + draw_plot(f1_legend, .2, .2, .6, .3,  scale=2), 
               bot,
               nrow=2, 
               rel_heights = c(2,1))
...
ggsave("supp/intra_sd_ideo_legend.pdf", width = 16*3/5*s, height = 1/24*12*s, plot=f1_legend)
ggsave("figures/Figure1.simple.pdf", plot=ftall, height = 12*s, width = 16*s)
...
make_gene_fam = c("GPRIN2"="GPRIN2",  "NPIPB4"="NPIP", "TBC1D3K"="TBC1D3", "NBPF15"="NBPF", "FRG2C"="FRG2C", "LPA"="LPA","SMN2"="SMN", "AMY1A"="AMY","GAGE12E"="GAGE")
