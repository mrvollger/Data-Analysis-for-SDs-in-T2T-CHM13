...
probe_plot = plot_grid(probe_l, NULL, flat_duplicon, nrow=3, rel_heights = c(1,0,8), scale = c(0.7, 1, 1));
probe_plot
...
probe_tbl= ggplot(data=results) + 
  geom_bar(aes(x=loc, fill=probe_id))+
  facet_grid(sample~chr, switch="y")+
  scale_fill_manual(values = colors)+
  theme_cowplot() + 
  ylab("")+
  ...
  theme(legend.position = "none", axis.text.y = element_blank(),
        plot.title = element_text(hjust = 0.5), 
        axis.text = element_text(angle = 60, vjust = 0.5, hjust=1)
        ) 
...
#
#
g_fish_imgs = ggdraw() + draw_image("FISH/fosmid_png.png")


top=plot_grid(p_duplicons, probe_plot, labels=c("a","b"), rel_widths = c(4,5))
figure = plot_grid(top, g_fish_imgs, labels = c(NA,"c"), rel_heights = c(5,2), nrow=2)

figure = plot_grid( plot_grid(flat_duplicon, g_fish_imgs, labels = c("a","b"), rel_widths = c(3,2)), p_duplicons,labels = c(NA,"c"), nrow=2)

#figure
ggsave("figures/duplicons.pdf", plot=figure , width=16, height=12) 
ggsave("figures/duplicons.png", plot=figure , width=16, height=12, dpi = 300) 
...
png("simple_inter.png", height = 6, width = 6, units="in", res=900, bg = "transparent")
Inter()
dev.off()
pdf("simple_inter.pdf", height = 6, width = 6)
Inter()
dev.off()

g_inter = ggdraw() + draw_image("simple_inter.png")
```
...
merged_sds = bind_rows(as.data.table(reduce(toGRanges(SEDEF))),
                       as.data.table(reduce(toGRanges(SEDEF_38))),
                       as.data.table(reduce(toGRanges(SEDEF_CELARA))),
                       .id="id")
...
#
#
ideo = plot_grid(Intra, g_inter,  labels = c("a","b"), rel_widths = c(1,1), ncol=2); 
#p = plot_grid(ideo, She, nrow=2)
f1 = plot_grid(ideo, sd_comparison, nrow=2, rel_heights = c(2,1))
